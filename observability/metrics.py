...
def update_ws_connection_count(count: int) -> None:
    WS_CONNECTIONS.set(count)


def record_ws_message_received(msg_type: str) -> None:
    WS_MESSAGES_RECEIVED.labels(type=msg_type).inc()


def record_ws_message_sent(msg_type: str) -> None:
    WS_MESSAGES_SENT.labels(type=msg_type).inc()


def record_ws_reconnection() -> None:
    WS_RECONNECTIONS.inc()


def record_ws_sequence_gap() -> None:
    WS_SEQUENCE_GAPS.inc()


def record_order_placed(ticker: str, side: str, action: str, status: str) -> None:
    ORDERS_PLACED.labels(ticker=ticker, side=side, action=action, status=status).inc()


def record_order_rejected(ticker: str, side: str, reason: str) -> None:
    ORDERS_REJECTED.labels(ticker=ticker, side=side, reason=reason).inc()


def record_order_latency(ticker: str, side: str, latency: float) -> None:
    ORDER_LATENCY.labels(ticker=ticker, side=side).observe(latency)


def update_positions_open(ticker: str, side: str, count: int) -> None:
    POSITIONS_OPEN.labels(ticker=ticker, side=side).set(count)


def update_position_pnl(ticker: str, side: str, pnl_type: str, value: float) -> None:
    POSITION_PNL.labels(ticker=ticker, side=side, pnl_type=pnl_type).set(value)


def update_total_capital(value: float) -> None:
    TOTAL_CAPITAL.set(value)


def update_sector_exposure(sector: str, value: float) -> None:
    SECTOR_EXPOSURE.labels(sector=sector).set(value)


def update_var_usage(percent: float) -> None:
    VAR_USAGE.set(percent)


def record_kill_switch_triggered() -> None:
...
def update_rate_limiter_tokens(tier: str, tokens: float) -> None:
    RATE_LIMITER_TOKENS.labels(tier=tier).set(tokens)


def update_dlq_size(name: str, size: int) -> None:
    DLQ_SIZE.labels(name=name).set(size)


def record_dlq_processed(name: str, status: str) -> None:
    DLQ_PROCESSED.labels(name=name, status=status).inc()


def update_system_memory(bytes_used: int) -> None:
    SYSTEM_MEMORY_USAGE.set(bytes_used)


def update_system_cpu(percent: float) -> None:
    SYSTEM_CPU_USAGE.set(percent)


def update_system_temperature(celsius: float) -> None:
    SYSTEM_TEMPERATURE.set(celsius)


def record_shadow_trade(ticker: str, side: str) -> None:
    SHADOW_TRADES.labels(ticker=ticker, side=side).inc()


def record_ws_latency(seconds: float) -> None:
    WS_LATENCY.set(seconds)


def update_ws_last_message_time(timestamp: float) -> None:
    WS_LAST_MESSAGE_TIME.set(timestamp)


def update_db_stats(stats: dict[str, Any]) -> None:
...
def record_strategy_trigger(indicator: str, signal: str) -> None:
    STRATEGY_TRIGGERS.labels(indicator=indicator, signal=signal).inc()


def push_to_gateway(
    gateway_url: str | None = None,
    job_name: str = "kalshi_bot",
