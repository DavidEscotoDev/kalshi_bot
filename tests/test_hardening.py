...
class TestCircuitBreakerHardening:
    def test_record_failure_manually(self) -> None:
        cb = CircuitBreaker(
            "test_rf", CircuitBreakerConfig(failure_threshold=3, timeout_seconds=60.0)
        )
        ...
        assert cb.state.value == "closed"
        cb.record_failure()
        assert cb.state.value == "open"  # type: ignore[comparison-overlap]

    def test_bypass_skips_open_check(self) -> None:
        cb = CircuitBreaker(
            "test_bypass", CircuitBreakerConfig(failure_threshold=1, timeout_seconds=60.0)
        )
    ...
    def test_bypass_still_records_failures(self) -> None:
        cb = CircuitBreaker(
            "test_bypass_fail", CircuitBreakerConfig(failure_threshold=2, timeout_seconds=60.0)
        )
    ...
    def test_record_failure_resets_after_half_open_success(self) -> None:
        cb = CircuitBreaker(
            "test_reset",
            CircuitBreakerConfig(failure_threshold=2, success_threshold=1, timeout_seconds=0.1),
        )
        cb.record_failure()
        cb.record_failure()
        assert cb.state.value == "open"
...
class TestRateLimiterHardening:
    def test_configure_default_tiers(self) -> None:
        rl = get_rate_limiter()
        configure_default_tiers()
    ...
    def test_configure_default_tiers_idempotent(self) -> None:
        rl = get_rate_limiter()
        configure_default_tiers()
        configure_default_tiers()
        stats = rl.get_all_stats()
...
class TestKillSwitchHardening:
    def test_cancel_all_orders_bypass_circuit(self) -> None:
        with (
    ...
    def test_check_and_trigger_with_capital_below_limit(self) -> None:
        with (
            patch("safety.kill_switch.Config.get_verified_session") as mock_session,
            patch("safety.kill_switch.Config.request_with_retry") as mock_request,
            patch("safety.kill_switch.Config.get_private_key") as mock_key,
            ...
            mock_request.return_value = MagicMock(
                status_code=200, json=lambda: {"balance_dollars": "50.00"}
            )
            from safety.kill_switch import KillSwitch
...
class TestOrderBookConcurrency:
    def test_concurrent_clear_and_apply(self) -> None:
        book = LocalOrderBook()
        ...
        def apply_snapshot_loop() -> None:
            for _ in range(100):
                try:
                    book.apply_snapshot(
                        {"msg": {"yes_dollars_fp": [["0.50", "10"]], "no_dollars_fp": []}}
                    )
                except Exception as e:
                    errors.append(e)
        ...
        def write_loop() -> None:
            for i in range(100):
                try:
                    ...
                    book.apply_snapshot(
                        {"msg": {"yes_dollars_fp": [[str(price), "5"]], "no_dollars_fp": []}}
                    )
                except Exception as e:
                    errors.append(e)
    ...
    def test_clear_while_delta(self) -> None:
        book = LocalOrderBook()
        book.apply_snapshot(
            {"msg": {"yes_dollars_fp": [["0.50", "10"]], "no_dollars_fp": [["0.40", "5"]]}}
        )
        errors: list[Exception] = []
        ...
        def delta_loop() -> None:
            for _ in range(50):
                try:
                    book.apply_delta(
                        {"msg": {"side": "yes", "price_dollars": "0.50", "delta_fp": "1"}}
                    )
                except Exception as e:
                    errors.append(e)
...
class TestPositionManagerHardening:
    ...
    def test_trailing_stop_no_trigger_within_tolerance(self) -> None:
        pm = PositionManager()
        pos = Position(
            ticker="TEST",
            side=OrderSide.YES,
    ...
    def test_take_profit_tier_1_triggers(self) -> None:
        with patch(
            "execution.position_manager.Config.TAKE_PROFIT_TIER_1_MULTIPLIER", Decimal("2.0")
        ):
            pm = PositionManager()
            pos = Position(
                ticker="TEST",
    ...
    def test_take_profit_tier_2_triggers(self) -> None:
        with (
            patch(
                "execution.position_manager.Config.TAKE_PROFIT_TIER_1_MULTIPLIER", Decimal("2.0")
            ),
            patch(
                "execution.position_manager.Config.TAKE_PROFIT_TIER_2_MULTIPLIER", Decimal("3.0")
            ),
        ):
            pm = PositionManager()
            pos = Position(
    ...
    def test_take_profit_marked_does_not_retrigger(self) -> None:
        pm = PositionManager()
        pos = Position(
            ticker="TEST",
            side=OrderSide.YES,
    ...
    def test_no_side_trailing_stop(self) -> None:
        pm = PositionManager()
        pos = Position(
            ticker="TEST",
            side=OrderSide.NO,
    ...
    def test_close_partial_updates_position(self) -> None:
        pos = Position(
            ticker="TEST",
            side=OrderSide.YES,
            quantity=Decimal("100"),
    ...
    def test_update_unrealized_tracks_highest_price(self) -> None:
        pos = Position(
            ticker="TEST",
            side=OrderSide.YES,
            quantity=Decimal("100"),
...
class TestShadowModeOverride:
    def test_shadow_mode_override_enabled(self) -> None:
        ...
        with patch("execution.engine.Config.SHADOW_MODE", False):
            ...
            effective = (
                engine._shadow_mode_override
                if engine._shadow_mode_override is not None
                else Config.SHADOW_MODE
            )
            assert effective is True

    def test_shadow_mode_override_disabled(self) -> None:
        from execution.engine import ExecutionEngine
    ...
    def test_shadow_mode_override_reset(self) -> None:
        from execution.engine import ExecutionEngine
    ...
    def test_shadow_mode_overrides_global(self) -> None:
        from execution.engine import ExecutionEngine
...
class TestConfigValidation:
    ...
    def test_validate_raises_on_bad_var_limit(self) -> None:
        save_key = Config._private_key
        Config._private_key = MagicMock()
        try:
            Config.MAX_VAR_LIMIT_PCT = Decimal("0")
            with (
                patch.dict(os.environ, {"KALSHI_TESTING": "0"}),
                pytest.raises(ValueError, match="MAX_VAR_LIMIT_PCT"),
            ):
                Config.validate()
        finally:
    ...
    def test_validate_raises_on_bad_sector_limit(self) -> None:
        save_key = Config._private_key
        Config._private_key = MagicMock()
        try:
            ...
            with (
                patch.dict(os.environ, {"KALSHI_TESTING": "0"}),
                pytest.raises(ValueError, match="MAX_SECTOR_LIMIT_PCT"),
            ):
                Config.validate()
        finally:
    ...
    def test_validate_raises_on_bad_kelly(self) -> None:
        save_key = Config._private_key
        Config._private_key = MagicMock()
        try:
            Config.MAX_VAR_LIMIT_PCT = Decimal("0.02")
            ...
            with (
                patch.dict(os.environ, {"KALSHI_TESTING": "0"}),
                pytest.raises(ValueError, match="KELLY_MULTIPLIER"),
            ):
                Config.validate()
        finally:
    ...
    def test_validate_skipped_in_testing(self) -> None:
        os.environ["KALSHI_TESTING"] = "1"
...
class TestHealthEndpoint:
    def test_ready_returns_503_when_circuit_breaker_open(self) -> None:
    ...
    def test_ready_returns_200_when_all_checks_pass(self) -> None:
        from starlette.testclient import TestClient
...
class TestAuditLogCleanup:
    def test_cleanup_removes_old_files(self, tmp_path: Any) -> None:
    ...
    def test_cleanup_keeps_recent_files(self, tmp_path: Any) -> None:
        old_dir = tmp_path / "audit"
        old_dir.mkdir()
...
class TestProfitabilityTracking:
    def test_update_signal_profitability_sets_flag(self) -> None:
        from data.database import (
            get_strategy_performance,
            initialize_db,
            log_strategy_signal,
            update_signal_profitability,
        )
    ...
    def test_update_signal_profitability_does_not_overwrite(self) -> None:
        from data.database import (
            get_strategy_performance,
            initialize_db,
            log_strategy_signal,
        ...
        sig_id = log_strategy_signal(
            "PCE", 2.0, 2.5, 0.5, 2.0, "strong", 0.80, "yes", 200.0, profitable=True
        )
        assert sig_id > 0
    ...
    def test_store_order_with_signal_id(self) -> None:
        from data.database import (
        ...
        order_id = store_order("test-sig-link", "TEST", signal_id=sig_id, outcome_side="yes")
        assert order_id > 0
    ...
    def test_on_order_fill_close_updates_profitability(self) -> None:
        from decimal import Decimal
...
class TestConfigSession:
    ...
    def test_session_has_connection_pooling(self) -> None:
