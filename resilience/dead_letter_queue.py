...
def _update_dlq_metric(name: str, size: int) -> None:
    try:
        from observability.metrics import update_dlq_size as _fn

        _fn(name, size)
    except Exception:
        logger.debug("Failed to update DLQ metric", exc_info=True)


def _record_dlq_processed(name: str, status: str) -> None:
    try:
        from observability.metrics import record_dlq_processed as _fn

        _fn(name, status)
    except Exception:
        logger.debug("Failed to record DLQ processed", exc_info=True)
...
class DeadLetterQueue:
    def __init__(
        ...
        self._queue: Queue[DLQEntry] = Queue()
    ...
    def add(self, payload: dict[str, Any], error: str, max_retries: int | None = None) -> str:
        ...
        logger.warning(f"DLQ '{self.name}': Added entry {entry_id} (queue size: {self.size()})")
        return entry_id
    ...
    def start_processor(self, interval: float = 30.0) -> None:
        if self._running:
            return
        self._running = True
    ...
    def _process_due_entries(self) -> None:
        now = time.time()
        processed = 0
        requeued = []
        ...
        while not self._queue.empty():
            ...
            if self.processor:
                try:
                    ...
                    logger.info(f"DLQ '{self.name}': Successfully processed entry {entry.id}")
                    self._remove_entry(entry.id)
                    processed += 1
                except Exception as e:
                    entry.retry_count += 1
                    backoff = min(self.base_backoff * (2**entry.retry_count), self.max_backoff)
                    entry.next_retry = now + backoff
                    entry.error = str(e)
                    self._persist_entry(entry)
                    requeued.append(entry)
                    logger.warning(
                        f"DLQ '{self.name}': Retry {entry.retry_count}/"
                        f"{entry.max_retries} for {entry.id} in {backoff}s: {e}"
                    )
            else:
                requeued.append(entry)
    ...
    def _move_to_failed(self, entry: DLQEntry) -> None:
...
class DLQRegistry:
    _queues: dict[str, DeadLetterQueue] = {}
    _instance = None
    _lock = threading.Lock()

    def __new__(cls) -> DLQRegistry:
    ...
    def get_or_create(self, name: str, **kwargs: Any) -> DeadLetterQueue:
        with self._lock:
    ...
    def start_all_processors(self, interval: float = 30.0) -> None:
        with self._lock:
